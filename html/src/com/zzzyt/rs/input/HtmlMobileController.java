...
public class HtmlMobileController extends GestureAdapter {
	...
	@Override
	public boolean tap(float x_raw, float y_raw, int count, int button) {
		if (b == null)
			this.b = RocketSimulator.rs.drawer.buttons;

		float h = RocketSimulator.rs.h;
		float w = RocketSimulator.rs.w;
		float y = Gdx.graphics.getHeight() - 1 - y_raw;
		float x = x_raw;
		scale = 0.5f;
		
		if (Math.min(h, w * 9 / 16) > 700)
			scale = 1f;
...
//			//spd
//		}
		if (inBox(x, y, w - 240 * scale, h - 80 * scale, 100, 100)) {
			// focus
			rs.focusOn = !rs.focusOn;
			b.Pfocus = !b.Pfocus;
		} else if (count == 3) {
			// stage
			if (rs.r.stage < rs.r.stages.size() - 1)
				rs.r.stage();
		} else {
			return false;
		}
